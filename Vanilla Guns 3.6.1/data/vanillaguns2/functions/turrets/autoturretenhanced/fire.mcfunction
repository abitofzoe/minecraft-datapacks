...
function vanillaguns2:turrets/autoturretenhanced/getcastamount

scoreboard players operation debug debug = @s VG_autoturcast

execute at @s[scores={VG_autoturammo=0,VG_autoturcast=1..}] run playsound minecraft:block.stone_button.click_on player @a ~ ~ ~ 0.5 2

execute if score @s[scores={VG_autoturammo=1..}] VG_autoturcast matches 1.. run function vanillaguns2:turrets/autoturretenhanced/castloop

execute as @s[scores={VG_autoturammo=1..}] at @s as @e[type=#vanillaguns2:hostileenhanced,distance=1..32,tag=VG_autoturtarget,limit=1,sort=nearest,tag=VG_autoturhit] if score @s VG_autoturid = @e[type=armor_stand,tag=VG_autoturret,distance=..0.1,limit=1] VG_autoturid as @e[type=armor_stand,tag=VG_autoturret,distance=..0.1,limit=1] run function vanillaguns2:turrets/autoturretenhanced/fireatwill

function vanillaguns2:turrets/autoturretenhanced/cleanup
